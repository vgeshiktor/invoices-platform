...
"""
Graph Invoice Finder — v3.7.4 (Full)

נקודות מפתח:
- AQS $search בלבד (בלי $filter), כדי להימנע משגיאות "SearchWithFilter".
- ללא @odata.type בתוך $select/expand (תיקון BadRequest).
- איתור חשבוניות גם דרך דפי ביניים (Bezeq וכו'): onclick / script / form / iframe (עומק 2).
- אימות PDF גמיש: none | light | strict (+override עם --verify-threshold).
- תמיכה ב־PyMuPDF (מועדף) ו־PyPDF2.
"""
...
LINK_RULES = {
    ...
    "allowed_http_providers": [
        # חלק מגופי עירייה עדיין http
        "citypay.co.il"
    ],
    "tracking_domains": [
        "webversion.net",
...
# ===== אסטרטגיות חיפוש (AQS) — ללא סוגריים חיצוניים מיותרים =====
STRATEGIES = [
    {
...
def crawl_html_for_pdf(
    ...
    *,
    ...
    # 3) ירידה לדפי ביניים/iframe (HTML)
    for u in candidates:
        lo = u.lower()
        if lo.endswith(".pdf"):
            continue
                ...
                for j, p in enumerate(candidate_list, start=1):
                    ...
                    if ok3:
                        ok4, meta4 = verify_pdf_is_invoice(
                            out2, threshold=verify_threshold
                        )
                        if ok4:
                            return out2, {
                                "download": meta3,
...
# ============== Providers AQS helper (ללא סוגריים) ==============
...
def main():
    ...
    p.add_argument(
        "--allow-http-providers", default=None, help="פסיק: דומיינים שמותרים גם http://"
    )
    ...
    p.add_argument(
        "--download-link-texts", default="להורדה,הורדה,download,צפייה,להצגה,view,open"
    )
    ...
    global DEBUG, PDF_ENGINE, deep_html, download_texts, try_forms, dump_html_dir
    DEBUG = args.debug
    ...
    # עדכון אסטרטגיית ספקים (ללא סוגריים)
    for s in STRATEGIES:
