...
- Tuning knobs:
  * DEFAULT_SENDER_BOOST includes '+@bezeq.co.il' (can be extended/replaced via CLI)
  * --negative-keywords "newsletter,promotion,..." (subtracts score on hits)
  * --sender-boost "+@good.com,-@bad.com,+exact@addr.com" (+ --sender-boost-mode extend|replace)
...
- Default trusted providers include "bezeq.co.il"; trust applies to subdomains (e.g., *.bezeq.co.il).
Outputs:
...
# NEW (v3.5.2): default sender boosts (extendable/replaceable via CLI)
...
def is_trusted_domain(dom: str, trusted_list: List[str]) -> bool:
    """Trust exact matches or any subdomain of a trusted root (e.g., *.bezeq.co.il)."""
    dom = (dom or "").lower()
...
def run_strategy(
    ...
    # NOTE: Typo fixed if you copy from earlier versions—should be /me/messages
    url = f"{GRAPH_BASE}/me/messages"
    params = {
...
def main():
    ...
    p.add_argument(
        ...
        default="extend",
        help="Extend (default) or replace the default sender boost list.",
    )
    p.add_argument(
        "--subject-regex",
        default=None,
    ...
    # Trusted providers (Bezeq already in defaults; subdomain trust handled by is_trusted_domain)
    ...
    # Sender boost rules: start from defaults, then extend/replace with CLI
    ...
    if args.sender_boost:
        ...
        if args.sender_boost_mode == "replace":
            ...
            # extend with de-dup while preserving order
    ...
    for mid, msg in seen.items():
        ...
        # Sender boost (now includes defaults)
