...
"""
Graph Invoice Finder (v3.3)
- $search without $filter/$orderby/$count (avoid SearchWithFilter & InefficientFilter)
- Date filtering happens client-side
- Each strategy has a human-readable description
- Prints summary table (per-strategy candidates & final matches)
- Exports results to JSON/CSV and summary to CSV
"""
...
def utc_now_minus_days(days: int) -> dt.datetime:
...
def main():
    ...
    since_dt = utc_now_minus_days(args.lookback_days)
    ...
    summary_rows = []
    header = f"\nStrategy summary (lookback={DEFAULTS['lookback_days']}d, min_conf={DEFAULTS['min_confidence']}):"
    print(header)
    print("-" * len(header))
