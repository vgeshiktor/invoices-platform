...
"""
Graph Invoice Finder (v3.7.3)

שינויים עיקריים:
- הורדת צרופות: בלי @odata.type ב-$select; תוכן fileAttachment דרך /$value.
- אימות PDF גמיש: --verify none|light|strict + --verify-threshold.
- פירוק HTML "עמוק" לקישורי/כפתורי הורדה (כולל onclick/form/meta-refresh), מתאים ל-myinvoice.bezeq.co.il.
- דגל --accept-octet-stream לקבצים שמוחזרים כ-application/octet-stream.
- סיכום סף-סף (--threshold-sweep) על תוצאות סופיות.

הרצה לדוגמה:
python graph_invoice_finder.v3.7.3.py \
...
DEBUG = False
PDF_ENGINE = "auto"  # יתעדכן אחרי argparse
...
VERIFY_DEFAULTS = {
    "none": 0.0,  # אין אימות – שמור כל PDF
    "light": 0.30,  # סף נמוך
    "strict": 0.45,  # סף "רגיש" (הקודם)
}
...
def _find_download_candidates_bs4(
    ...
    for a in soup.find_all("a"):
        t = _collect_node_text(a)
        if any(x in t for x in texts):
            href = a.get("href")
            if href:
                full = urljoin(base, href)
                urls.append(full)
                dbg.append({"kind": "a[text]", "text": t, "href": href, "full": full})
        # onclick
        oc = a.get("onclick")
        for u in _extract_urls_from_onclick_attr(oc or "", base):
            urls.append(u)
            dbg.append({"kind": "a[onclick]", "onclick": oc, "full": u})

    # 2) <button> / <input type=button|submit>
    for tag in soup.find_all(["button", "input"]):
        tt = tag.name.lower()
        t = _collect_node_text(tag) if tt == "button" else _text_norm(tag.get("value"))
        if any(x in (t or "") for x in texts):
            data_url = tag.get("data-url") or tag.get("href") or tag.get("formaction")
            if data_url:
                full = urljoin(base, data_url)
                urls.append(full)
                dbg.append({"kind": f"{tt}[data-url]", "full": full})
            oc = tag.get("onclick")
            for u in _extract_urls_from_onclick_attr(oc or "", base):
                urls.append(u)
                dbg.append({"kind": f"{tt}[onclick]", "onclick": oc, "full": u})
    ...
    # 4) script: חפש window.open/location=...
    for s in soup.find_all("script"):
        sc = s.string or s.text or ""
        for u in _extract_urls_from_onclick_attr(sc, base):
            urls.append(u)
            dbg.append({"kind": "script", "full": u})
...
def find_deep_download_urls(
    ...
    *,
    ...
    urls = []
    if HAS_BS4:
        found, dbg = _find_download_candidates_bs4(html, page_url, texts)
        if DEBUG:
            print("[deep-html] candidates:", json.dumps(dbg, ensure_ascii=False))
        urls.extend(found)
        if try_forms:
            subs = _find_and_submit_download_forms(sess, html, page_url, texts)
            for fin, info in subs:
                if fin:
                    urls.append(fin)
                    if DEBUG:
                        print("[deep-html] form-submitted:", info)
    else:
        # רגרסיה ללא BS4
        for u in _extract_urls_from_onclick_attr(html, page_url):
            urls.append(u)
        for m in re.finditer(
            r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>(.*?)</a>', html, re.I | re.S
        ):
            url, inner = m.group(1), _text_norm(unescape(m.group(2)))
            if any(x in inner for x in texts):
    ...
    *,
    ...
    if why in ("not_pdf", "ok"):
        try:
            ...
            # שלב 1: PDF ישירים בתוך ה-HTML
            pdf_links = find_pdf_links_in_html(r.text or "", r.url)

            # שלב 2: „חפירה עמוקה” לכפתורי/קישורי הורדה (למשל 'להורדה' בבזק)
            deep_links = []
            if globals().get("deep_html", True):
                dl_texts = globals().get(
                    "download_texts",
                    ["להורדה", "download", "הורדה", "view", "צפייה", "להצגה", "open"],
                )
                deep_links = find_deep_download_urls(
                    ...
                    r.text or "",
                    dl_texts,
                    try_forms=globals().get("try_forms", True),
                )
...
def main():
    ...
    p.add_argument(
        "--invoices-dir",
        required=False,
        help="תיקיית יעד – רק חשבוניות מאומתות יישמרו כאן. אם לא יוגדר, תיווצר אחת עם timestamp.",
    )
    p.add_argument(
        "--keep-quarantine",
        action="store_true",
        help="שמור קבצים שנדחו (לא חשבוניות) תחת invoices_dir/quarantine לצורך דיבוג.",
    )
    p.add_argument(
        "--download-timeout",
        type=int,
    ...
    p.add_argument(
        "--threshold-sweep",
        default=None,
        help="רשימת ספים מופרדת בפסיקים לניתוח כמו: 0.30,0.40,0.55,0.70",
    )
    ...
    p.add_argument(
        ...
        default="auto",
        help="מנוע חילוץ טקסט מ-PDF: pymupdf | pypdf | auto",
    )
    ...
    p.add_argument(
        ...
        default="strict",
        help="מצב אימות PDF: none (ללא), light (סף נמוך), strict (דיפולט).",
    )
    p.add_argument(
        "--verify-threshold",
        type=float,
    ...
    p.add_argument(
        "--accept-octet-stream",
        action="store_true",
        help="החשבת application/octet-stream כתקין אם הדומיין מותר/מהימן.",
    )
    p.add_argument(
        "--download-link-texts",
        default="להורדה,הורדה,download,צפייה,להצגה,view,open",
        help="מילות מפתח לזיהוי קישור/כפתור הורדה בעמוד HTML (פסיקים).",
    )
    p.add_argument(
        "--no-deep-html",
        action="store_true",
        help="בטל חיפוש מתקדם בעמוד (onclick/forms/meta-refresh).",
    )
    p.add_argument(
        "--try-form-submit",
        action="store_true",
        help="אפשר שליחת טפסים שמכילים כפתור הורדה (ברירת מחדל: פעיל).",
    )
    p.add_argument(
        "--no-try-form-submit", action="store_true", help="בטל שליחת טפסים (override)."
    )
    p.set_defaults(try_form_submit=True)  # דיפולט: כן
    ...
    # התנהגות HTML עמוק
    global deep_html, download_texts, try_forms
    ...
    # Threshold sweep (על השורות הסופיות שנשמרו)
