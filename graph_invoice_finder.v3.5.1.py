...
- Tuning knobs:
  ...
  * --subject-regex "Invoice\\s*#\\d+,חשבונית(\\ס*מס)?\\s*#?\\d+" (boost on match)
...
- Default trusted providers now include "bezeq.co.il", and trust applies to subdomains (e.g., *.bezeq.co.il).
Outputs:
...
def main():
    ...
    # Tune: providers (Bezeq is already in defaults; subdomain trust handled by is_trusted_domain)
