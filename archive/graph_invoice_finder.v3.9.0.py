...
"""
Graph Invoice Finder — v3.9 (Full, with robust Bezeq Playwright handler)

========================================
README + CLI COOKBOOK (נא לקרוא פעם אחת)
========================================

מטרה
-----
לאתר מיילים עם חשבוניות/קבלות (עברית/אנגלית) ב-Outlook/Exchange Online באמצעות Microsoft Graph,
להוריד קבצי PDF מצורפים או לינקים שמובילים ל-PDF (כולל דפים דינמיים כמו myinvoice.bezeq.co.il),
ולבצע אימות בסיסי שהקובץ הוא אכן חשבונית. התוצרים נשמרים לתיקייה מקומית, עם דוח הורדות.

מה חדש בגרסה v3.9
------------------
1) **תיקון Playwright לבזק**:
   - זיהוי כפתור “להורדה/Download” בכמה סלקטורים + המתנה אקטיבית.
   - תמיכה ב-popup/new-tab.
   - לכידת תגובות PDF **inline** (גם כשאין download event) ושמירת ה-body לקובץ.
   - דיבוג משופר (notes/pdf_url/screenshot).
2) ממשיך להימנע מ-`$filter` מול `$search` (AQS בלבד) כדי לא לקבל `SearchWithFilter`.
3) הורדת צרופות דרך נתיב `/$value` (ללא `@odata.type` ב-$select) כדי לא לקבל שגיאת OData.
4) אימות PDF: `--verify none|light|strict` או `--verify-threshold` מספרי. ברירת מחדל strict=0.45.
5) מנוע PDF: `--pdf-engine pymupdf` (מומלץ, מהיר), `pypdf` או `auto`.
6) תיעוד מלא + מתכוני הרצה מובנים כאן בקובץ.
...
בסיסי (טווח תאריכים):
    python graph_invoice_finder.v3.9.py \
      ...
      --verify strict --verify-threshold 0.45

עם דיבוג, הסברים, דוחות נוספים:
    python graph_invoice_finder.v3.9.py \
      --client-id "YOUR-CLIENT-ID" \
      --authority consumers \
      --start-date 2025-09-01 --end-date 2025-10-01 \
      ...
      --keep-quarantine \
      --explain --debug \
      --dump-html-dir bezeq_html_dumps \
      --save-candidates raw_candidates.json \
      --save-nonmatches rejected.json \
      --threshold-sweep 0.30,0.40,0.55,0.70

בזק (Flutter) עם פעולה ידנית אם צריך (Captcha/כניסה):
    python graph_invoice_finder.v3.9.py \
      --client-id "YOUR-CLIENT-ID" \
      --authority consumers \
      --start-date 2025-09-01 --end-date 2025-10-01 \
      ...
      --invoices-dir invoices_out \
      --verify strict \
      --interactive --no-browser-headless \
      --dump-html-dir bezeq_html_dumps \
      --debug

סוויצ'ים חשובים
----------------
--interactive              לאפשר פעולה ידנית בדפדפן (Captcha/כניסה).
--no-browser-headless      להציג דפדפן (לראות מה קורה). ברירת מחדל: headless.
--dump-html-dir DIR        לשמור HTML לצורך דיבוג (שורש ודפים פנימיים).
--keep-quarantine          לא למחוק קבצים שלא עברו אימות — לשים ב-quarantine.
--accept-octet-stream      לאפשר הורדת PDF גם אם content-type הוא application/octet-stream.
--providers                להוסיף דומיינים "מהימנים" ללינקים (פסיק-מופרד).
--allow-http-providers     לאפשר ספקים ספציפיים על http (למשל citypay.co.il).

תוצרים
------
- תיקיית חשבוניות (ברירת מחדל invoices_final_YYYYMMDD_HHMMSS או כבקשתך ב--invoices-dir)
- report: invoices_dir/download_report.json   (saved/rejected עם סיבות)
- מסרים שעברו אימות: invoice_messages_*.json / *.csv
- סיכום אסטרטגיות: invoices_summary_*.csv
- (אופציונלי) raw_candidates.json + rejected.json + HTML dumps + screenshots

טרבל־שוטינג (בזק)
-----------------
- ראה ב-download_report.json: אם מופיע `bezeq_playwright_no_pdf` או `download_captured=false`,
  הרץ עם `--interactive --no-browser-headless`. בצילומי המסך (bezeq_screen_*.png) תוכל
  לוודא שהכפתור קיים וגלוי. אם PDF נטען inline, הפונקציה החדשה תופסת אותו ב-on_response.
- אם עדיין אין PDF: שלח את ה-notes/pdf_url מהרפורט ונרחיב את הסלקטורים/המתנות.

זכורות והעדפות
---------------
- לבקשת המשתמש: תמיד לכלול תיעוד מלא ופקודות הרצה בקובץ .py (או .md נלווה) — הוטמע.
"""
...
PDF_ENGINE = "auto"
deep_html = True
download_texts: List[str] = []
try_forms = True
dump_html_dir: Optional[str] = None

# Playwright flags
PLAYWRIGHT_AVAILABLE = None  # lazy init
...
VERIFY_DEFAULTS = {"none": 0.0, "light": 0.30, "strict": 0.45}
...
LINK_RULES = {
    ...
    "allowed_http_providers": ["citypay.co.il"],  # יש עיריות עם http
    "tracking_domains": [
        "webversion.net",
        "web-view.net",
        "mandrillapp.com",
...
STRATEGIES = [
    ...
    },
    {
        ...
        "description": "רשת רחבה יותר כולל statement/billing/תשלום — מועיל לחשבוניות-לינק שמתחמקות מהמילה 'חשבונית'.",
    ...
    },
    {
        ...
        "description": "כיוון לספקים מוכרים (GreenInvoice, iCount, QuickBooks, Stripe, tax.gov.il, Cardcom, Bezeq).",
        "search": '"greeninvoice.co.il" OR "icount.co.il" OR "ezcount.co.il" OR "quickbooks.intuit.com" OR "stripe.com" OR "paypal.com" OR "zoho.com" OR "xero.com" OR "shopify.com" OR "tax.gov.il" OR "cardcom.co.il" OR "myinvoice.bezeq.co.il" OR "bezeq.co.il"',
    },
    {
...
def language_confidence(
    ...
    he = KEYWORDS["hebrew_core"] + KEYWORDS["hebrew_context"]
    en = KEYWORDS["english_core"] + KEYWORDS["english_context"]
    blob = f"{subject}\n{preview}\n{body_text}".lower()
    hits_he = count_hits(blob, he)
    hits_en = count_hits(blob, en)
...
def score_links(
    ...
    for u in urls:
        ...
        if not is_https and not (
            u_low.startswith("http://") and is_http_allowed(dom, allowed_http_providers)
        ):
            continue
        ...
        if tracky and not (dom_trust or has_kw or has_pat or has_suf):
            continue
    ...
    if any_kw_or_pat:
        score += 0.15
    return list(dict.fromkeys(kept)), min(score, 0.7)
...
def donation_bonus(subj: str) -> float:
    s = (subj or "").lower()
    if "קבלה" in s and any(
        t in s for t in [x.lower() for x in KEYWORDS["donation_he"]]
    ):
        return 0.2
    return 0.0
...
def extract_pdf_text_first_pages(path: str, max_pages: int = 3) -> str:
...
# ---------- HTTP helpers / HTML crawling ----------
def requests_session() -> requests.Session:
    s = requests.Session()
    s.headers.update(
        {
            "User-Agent": "Mozilla/5.0 (InvoiceFetcher/1.0)",
            "Accept": "*/*",
...
def dump_html(dump_dir: Optional[str], base_name: str, url: str, html: str):
    if not dump_dir:
        return
    fname = safe_filename(base_name)[:120] + ".html"
...
# ----- HTML deep search (סטטי) -----
_ONCLICK_URL_RE = re.compile(
    r"(?:window\.open|location(?:\.href)?|document\.location)\s*\(\s*['\"]([^'\"]+)['\"]\s*\)|"
    r"(?:location(?:\.href)?|document\.location)\s*=\s*['\"]([^'\"]+)['\"]",
    re.I,
)
_GENERIC_QUOTED_URL_RE = re.compile(
    r"['\"]((?:https?:)?//[^'\"\s<]+|/[^\s'\"\)<>]+)['\"]", re.I
)
_STRONG_URL_HINTS = ("pdf", "invoice", "download", "receipt", "חשבונית", "קבלה")


def _extract_urls_from_onclick_attr(attr: str, base: str) -> List[str]:
    if not attr:
        return []
    s = unescape(attr)
    out = []
    for m in _ONCLICK_URL_RE.finditer(s):
        url = m.group(1) or m.group(2)
        if url:
            out.append(urljoin(base, url.strip()))
...
def _collect_node_text(node) -> str:
    try:
        return _text_norm(node.get_text(" ", strip=True))
    except Exception:
        return ""


def _attrs_to_urls(tag, base: str) -> List[str]:
    urls = []
    for attr in (
        "href",
        "src",
        ...
        v = tag.get(attr)
        if v:
            urls.append(urljoin(base, v))
    return urls


def _find_download_candidates_bs4(
    html: str, base: str, texts: List[str]
) -> Tuple[List[str], List[Dict[str, str]]]:
    urls, dbg = [], []
    soup = BeautifulSoup(html, "html.parser")

    def is_dl_text(tag) -> bool:
        t = _collect_node_text(tag)
        lbl = (
            _text_norm(tag.get("title") or "")
            + " "
            + _text_norm(tag.get("aria-label") or "")
        )
        return any(x in t for x in texts) or any(x in lbl for x in texts)

    for tag in soup.find_all(["a", "button", "input"]):
        if is_dl_text(tag):
            for u in _attrs_to_urls(tag, base):
                urls.append(u)
                dbg.append({"kind": f"{tag.name}[attrs]", "full": u})
        oc = tag.get("onclick")
        for u in _extract_urls_from_onclick_attr(oc or "", base):
            urls.append(u)
            dbg.append({"kind": f"{tag.name}[onclick]", "full": u})

    for m in soup.find_all("meta"):
        if (m.get("http-equiv") or "").lower() == "refresh":
            content = m.get("content") or ""
            mt = re.search(r"url\s*=\s*([^;]+)$", content, re.I)
            if mt:
                full = urljoin(base, mt.group(1).strip())
                urls.append(full)
                dbg.append({"kind": "meta-refresh", "full": full})

    for s in soup.find_all("script"):
        sc = s.string or s.text or ""
        for m in _GENERIC_QUOTED_URL_RE.finditer(sc):
            u = urljoin(base, m.group(1))
            lo = u.lower()
            if any(h in lo for h in _STRONG_URL_HINTS):
                urls.append(u)
                dbg.append({"kind": "script-url", "full": u})
    ...
    for ifr in soup.find_all("iframe"):
        ...
        if src:
            ...
            urls.append(full)
            dbg.append({"kind": "iframe", "full": full})

    urls = list(dict.fromkeys(urls))
    return urls, dbg


def _find_and_submit_download_forms(
    sess: requests.Session, html: str, base: str, texts: List[str]
) -> List[Tuple[str, Dict]]:
    out = []
    soup = BeautifulSoup(html, "html.parser")
    for form in soup.find_all("form"):
        ...
        for bt in form.find_all(["button", "input"]):
            txt = (
                _collect_node_text(bt)
                if bt.name.lower() == "button"
                else _text_norm(bt.get("value"))
            )
            if any(x in (txt or "") for x in texts):
        ...
        try:
            ...
            out.append(
                (r.url, {"method": method, "action": url, "status": r.status_code})
            )
        except Exception as e:
            out.append(("", {"method": method, "action": url, "error": str(e)}))
    return out
...
def crawl_html_for_pdf(
    ...
    *,
    ...
    pdf_links = find_pdf_links_in_html(html, page_url)
    found.extend(pdf_links)

    if HAS_BS4:
        candidates, dbg = _find_download_candidates_bs4(html, page_url, texts)
        if DEBUG:
            print(
                "[deep-html] depth",
                depth,
    ...
    direct_like = [
        ...
        or any(
            h in u.lower()
            for h in ("download", "invoice", "receipt", "חשבונית", "קבלה")
        )
    ]
    found.extend(direct_like)
    ...
    for u in candidates:
        ...
        try:
            r = sess.get(
                ...
                allow_redirects=True,
                timeout=DEFAULTS["http_timeout"],
                headers={"Referer": referer or page_url},
            )
            ct = (r.headers.get("Content-Type") or "").lower()
            if "text/html" in ct and r.status_code < 400:
                dump_html(dump_dir, f"depth{depth}_{domain_of(u)}", r.url, r.text or "")
                found.extend(
                    crawl_html_for_pdf(
                        sess,
                        r.url,
    ...
    if try_forms and HAS_BS4:
        subs = _find_and_submit_download_forms(sess, html, page_url, texts)
        for fin, _info in subs:
            if fin:
                found.append(fin)
...
def find_deep_download_urls(
    ...
    *,
    ...
    return crawl_html_for_pdf(
        sess, page_url, html, texts, try_forms=try_forms, depth=2, dump_dir=dump_dir
    )


# ---------- Attachments via /$value ----------
    ...
    *,
    ...
    if include_explain:
        row["explanations"] = {
            ...
            "links": {
                "candidates": extract_urls_from_body(msg.get("body") or {}),
                "kept": kept_links,
                "score": link_score,
            },
            "attachments": {"kept": attach_hits, "count": len(attach_hits)},
        }
    return row


# ---------- Provider handlers (Playwright) ----------
BEZEQ_HOSTS = {"myinvoice.bezeq.co.il"}


def is_bezeq_invoice_url(url: str) -> bool:
    host = domain_of(url)
    return any(host == h or host.endswith("." + h) for h in BEZEQ_HOSTS)


def ensure_playwright_imported():
...
def download_bezeq_with_playwright(
    url: str,
    out_dir: Path,
    *,
    headless: bool,
    ...
    """
    פותח את דף בזק, מאתר כפתור 'להורדה'/Download, לוכד גם PDF inline (response), שומר לקובץ.
    מכסה גם popup/new-tab. דורש playwright + chromium מותקנים.
    """
    ...
    def sanitize_filename(name: str) -> str:
        ...
        return name.strip() or "invoice.pdf"

    def save_pdf_response(resp, target_dir: Path) -> Optional[Path]:
        # שמירת גוף תגובת PDF (גם אם inline)
        try:
            ct = (resp.headers.get("content-type") or "").lower()
            if "application/pdf" not in ct:
                return None
            data = resp.body()
            if not data or len(data) < DEFAULTS["min_pdf_size_bytes"]:
                return None
            name = "invoice.pdf"
            ...
            if m:
                ...
                tail = urlparse(resp.url).path.rsplit("/", 1)[-1] or "invoice.pdf"
                if tail.lower().endswith(".pdf"):
                    name = tail
            path = target_dir / sanitize_filename(name)
            with open(path, "wb") as f:
                f.write(data)
            return path
        except Exception as e:
            diag["notes"].append(f"save_pdf_response_error:{e}")
            return None
    ...
    with sync_playwright() as p:
        ...
        def on_response(resp):
            ...
            if "application/pdf" in ct:
                diag["responses_pdf"] += 1
                path = save_pdf_response(resp, out_dir)
                if path:
                    diag["download_captured"] = True
                    diag["pdf_url"] = resp.url

        context.on("response", on_response)

        page.goto(url, wait_until="domcontentloaded", timeout=timeout_ms)
        page.wait_for_timeout(2000)  # bootstrap ל-Flutter

        has_captcha = page.locator("iframe[src*='recaptcha']").count() > 0
        if (has_captcha or "captcha" in page.content().lower()) and interactive:
            ss = out_dir / "bezeq_screen_before_click.png"
            page.screenshot(path=str(ss), full_page=True)
            diag["notes"].append(f"screenshot_saved:{ss}")
        ...
        if not found:
            tloc = page.get_by_text(re.compile("להורדה|הורד|Download", re.I))
            if tloc.count() > 0 and tloc.first.is_visible():
                found = tloc.first

        def try_click_and_capture(target):
            # 1) הורדה קלאסית
            try:
                ...
                path = out_dir / sanitize_filename(suggested)
                dl.save_as(str(path))
                diag["clicked"] = True
                diag["download_captured"] = True
                return path
            except Exception as e:
                diag["notes"].append(f"expect_download_failed:{e}")
            # 2) popup/new-tab
            try:
                with context.expect_page(timeout=4000) as pinfo:
                    target.click()
                newp = pinfo.value
                diag["popup_used"] = True
                newp.wait_for_load_state("domcontentloaded", timeout=10000)
                newp.wait_for_timeout(3000)
            except Exception:
                pass
            # 3) ניסיון נוסף
            try:
        ...
        if found:
            saved_path = try_click_and_capture(found)

        # חפש קישורי PDF ישירים בדף
        if not saved_path:
            ...
            if pdf_links.count() > 0:
                try:
                    ...
                    dl = dl_info.value
                    path = out_dir / sanitize_filename(
                        dl.suggested_filename or "invoice.pdf"
                    )
                    dl.save_as(str(path))
                    saved_path = path
                    diag["download_captured"] = True
                except Exception as e:
                    diag["notes"].append(f"pdf_link_click_failed:{e}")

        if not saved_path and not diag["download_captured"]:
            ss2 = out_dir / "bezeq_screen_no_pdf.png"
            page.screenshot(path=str(ss2), full_page=True)
            diag["notes"].append(f"no_pdf_found_post_bootstrap; screenshot:{ss2}")

        browser.close()

        if not saved_path and diag["download_captured"]:
            candidates = sorted(
                out_dir.glob("*.pdf"), key=lambda p: p.stat().st_mtime, reverse=True
            )
            if candidates:
                saved_path = candidates[0]

        return (saved_path, diag) if saved_path else (None, diag)


# ---------- Link fetch orchestrator ----------
def fetch_invoice_from_link(
    ...
    *,
    ...
    # 1) ניסיון ישיר (.pdf או ct=pdf/octet-stream)
    ...
    # 2) Bezeq (Playwright)
    if (
        is_bezeq_invoice_url(u)
        and bezeq_use_playwright
        and ensure_playwright_imported()
    ):
        try:
            out_path, diag = download_bezeq_with_playwright(
                u,
                Path(base_dir),
                headless=headless,
            ...
            if out_path:
                ok4, meta4 = verify_pdf_is_invoice(
                    str(out_path), threshold=verify_threshold
                )
                if ok4:
                    return str(out_path), {
                        "download": {"ct": "application/pdf"},
                        "verify": meta4,
                        "mode": "bezeq_playwright",
                    }
                else:
                    if os.path.exists(out_path):
                        os.remove(out_path)
                    return None, {"reject": f"verify_fail:{meta4}", **diag}
            else:
                return None, {"reject": "bezeq_playwright_no_pdf", **diag}
        except Exception as e:
            return None, {"reject": f"bezeq_playwright_exc:{e}"}

    # 3) HTML סטטי: הורד דף ושוטט לעומק
    try:
        ...
        dump_html(dump_html_dir, f"root_{domain_of(r.url)}", r.url, r.text or "")
        pdf_links = find_pdf_links_in_html(r.text or "", r.url)
        deep_links = []
        if deep_html:
            dl_texts = download_texts or [
                "להורדה",
                "הורד",
                "download",
                "view",
            ...
            deep_links = find_deep_download_urls(
                sess,
                r.url,
                r.text or "",
                dl_texts,
                try_forms=try_forms,
                dump_dir=dump_html_dir,
            )

        for candidate_list, mode_tag in (
            (pdf_links, "html_found_pdf"),
            (deep_links, "html_button_download"),
        ):
            for j, p in enumerate(candidate_list, start=1):
...
def main():
    p = argparse.ArgumentParser(
        description="Find & download invoice PDFs (Heb/Eng) via Microsoft Graph + Bezeq Playwright"
    )
    ...
    p.add_argument("--invoices-dir", required=False)
    p.add_argument("--keep-quarantine", action="store_true")
    p.add_argument("--download-timeout", type=int, default=DEFAULTS["http_timeout"])
    ...
    p.add_argument("--providers", default=None, help="פסיק: דומיינים מהימנים ללינקים")
    p.add_argument("--providers-mode", choices=["extend", "replace"], default="extend")
    p.add_argument("--allow-http-providers", default=None)

    p.add_argument("--pdf-engine", choices=["auto", "pymupdf", "pypdf"], default="auto")
    p.add_argument("--verify", choices=["none", "light", "strict"], default="strict")
    p.add_argument("--verify-threshold", type=float, default=None)
    p.add_argument("--accept-octet-stream", action="store_true")

    p.add_argument(
        "--download-link-texts",
        default="להורדה,הורד,הורדה,download,צפייה,להצגה,view,open",
    )
    p.add_argument("--no-deep-html", action="store_true")
    p.add_argument("--try-form-submit", action="store_true")
    p.add_argument("--no-try-form-submit", action="store_true")
    p.set_defaults(try_form_submit=True)

    p.add_argument(
        "--dump-html-dir",
        default=None,
        help="אם הוגדר, ישמור HTML ביניים לקבצים לצורך דיבוג.",
    )
    ...
    p.add_argument(
        "--interactive",
        action="store_true",
        help="לאפשר פעולה ידנית (captcha/כניסה) בחלון דפדפן.",
    )
    p.add_argument(
        "--browser-headless",
        dest="browser_headless",
        action="store_true",
        help="להריץ דפדפן ללא UI (ברירת מחדל).",
    )
    p.add_argument(
        "--no-browser-headless",
        dest="browser_headless",
        action="store_false",
        help="להציג את הדפדפן.",
    )
    p.set_defaults(browser_headless=True)
    ...
    deep_html = not args.no_deep_html
    download_texts = [
        t.strip().lower()
        for t in (args.download_link_texts or "").split(",")
        if t.strip()
    ]
    try_forms = bool(args.try_form_submit) and not bool(args.no_try_form_submit)
    dump_html_dir = args.dump_html_dir
    if dump_html_dir:
        ensure_dir(dump_html_dir)

    BROWSER_HEADLESS = args.browser_headless
    INTERACTIVE = args.interactive

    if PDF_ENGINE == "pymupdf" and not HAS_PYMUPDF:
        print("⚠️ PyMuPDF (fitz) לא מותקן — נופל ל-PyPDF2 (אם זמין).", file=sys.stderr)
        if HAS_PYPDF:
            PDF_ENGINE = "pypdf"
        else:
            PDF_ENGINE = "auto"
    if PDF_ENGINE == "pypdf" and not HAS_PYPDF:
        print("⚠️ PyPDF2 לא מותקן — נופל ל-PyMuPDF (אם זמין).", file=sys.stderr)
        if HAS_PYMUPDF:
            PDF_ENGINE = "pymupdf"
        else:
            PDF_ENGINE = "auto"
    ...
    # Providers (trusted/allowed http)
    ...
    subject_regexes: List[re.Pattern] = []
    ...
    for s in STRATEGIES:
        if s["name"] == "provider_whitelist_bump":
            aqs = build_provider_aqs(trusted)
            s["search"] = aqs or s["search"]
    ...
    for strat in STRATEGIES:
        ...
        msgs = run_strategy(
            gc, strat, args.page_size, args.max_pages, DEFAULTS["select_fields"]
        )
    ...
    download_log: Dict[str, Any] = {
        ...
        "browser": {
            "playwright_possible": ensure_playwright_imported(),
    ...
    download_report_json = os.path.join(invoices_dir_abs, "download_report.json")
    ...
    if args.threshold_sweep:
        try:
            ...
            rows_conf = [r["match_confidence"] for r in rows]
            for t in sorted(thre):
                cnt = sum(1 for c in rows_conf if c >= t)
                print(f"  >= {t:.2f}: {cnt} rows")
        except Exception as e:
            print(f"threshold_sweep parse error: {e}")

    print(f"\nSaved {len(rows)} messages that yielded verified invoices")
    print(f"Final invoices dir: {invoices_dir_abs}")
    print(f"Download report: {download_report_json}")
    print(f"JSON: {os.path.abspath(out_json)}")
    print(f"CSV : {os.path.abspath(out_csv)}")
    print(f"Summary: {os.path.abspath(out_summary_csv)}")
    if args.keep_quarantine:
        print(
            f"Quarantine kept under: {os.path.abspath(os.path.join(invoices_dir_abs, 'quarantine'))}"
        )
