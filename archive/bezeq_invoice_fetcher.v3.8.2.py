...
# -*- coding: utf-8 -*-
"""
bezeq_invoice_fetcher_v38_2.py

תכלית:
- להוריד חשבונית PDF מקישור myinvoice.bezeq.co.il (בזק) שמרנדר Flutter/PDF.js.
- כולל התמודדויות עם הורדת blob/new-tab, reCAPTCHA v3, ו־PDF.js viewer.

איך מריצים (דוגמאות):
    # ראשוני עם UI, טרייס וצילומי מסך
    python bezeq_invoice_fetcher_v38_2.py \
      --url "https://myinvoice.bezeq.co.il/?MailID=..." \
      --out-dir "./invoices_out_bezeq" \
      --headful --trace --screenshots

    # שקט (ברירת מחדל): headless + trace + screenshots
    python bezeq_invoice_fetcher_v38_2.py \
      --url "https://myinvoice.bezeq.co.il/?MailID=..." \
      --out-dir "./invoices_out_bezeq"
...
מה חדש בגרסה 38.2:
- תמיכה ב־--trace וגם --no-trace (תאימות לאחור), עם ברירת מחדל trace=ON.
- תמיכה סימטרית גם ל־--screenshots/--no-screenshots ו־--headful/--headless.
- שאר התיקונים מ־38.1 נשמרו: safe_screenshot עם Fallbacks, קליקים מרובי־אסטרטגיה,
  לכידת Download, חלונות popup ו־Response מסוג PDF, ועוד לוגים/trace.
"""
...
def main():
    ap = argparse.ArgumentParser(
        description="Bezeq (myinvoice) PDF fetcher – Flutter/Blob/PDF.js aware (v38.2)"
    )
    ap.add_argument(
        "--url",
        required=True,
    ...
    g_head.add_argument(
        ...
        dest="headful",
        action="store_true",
        help="פתח דפדפן עם UI (נוח לדיבוג)",
    )
    g_head.add_argument(
        "--headless",
        dest="headful",
    ...
    g_trace.add_argument(
        ...
        dest="trace",
        action="store_true",
        help="הפעל יצירת playwright trace (בררת מחדל)",
    )
    g_trace.add_argument(
        "--no-trace", dest="trace", action="store_false", help="כבה יצירת trace"
    )
