...
"""
Graph Invoice Finder (v3.6)
שינויים עיקריים:
- אסטרטגיית חיפוש ייעודית לעיריות/ארנונה (עברית).
- תמיכה ב-HTTP מותר לדומיינים מוגדרים (ברירת־מחדל + CLI).
- בונוס חכם לצרופות PDF מספריות בשילוב נושא "עירייה/ארנונה/שובר".

נשארו כל הפיצ'רים מהגרסאות הקודמות:
- $search בלבד (ללא $filter/$orderby/$count) כדי להימנע מ-SearchWithFilter/InefficientFilter.
- חלון תאריכים בצד לקוח: --start-date/--end-date או --lookback-days.
- דגלי אימות/ניטור: --explain --debug --save-candidates --save-nonmatches --threshold-sweep.
- כוונון ניקוד: negative keywords, sender boost (עם ברירת־מחדל +@bezeq.co.il), subject regex, providers, הורדת צרופות.

פלט:
- JSON + CSV של התאמות סופיות, ו-CSV סיכום לפי אסטרטגיה.
"""
...
# רשימת ספקים מהימנים (כולל bezeq.co.il)
LINK_RULES = {
    ...
    # חדש: דומיינים עם HTTP מותר (ללא https)
    "allowed_http_providers": ["citypay.co.il"],
}

LANG_WEIGHTS = {"he": 1.2, "en": 1.0}

# אסטרטגיות חיפוש ($search) — ללא $filter/$orderby/$count!
STRATEGIES = [
    {
...
def parse_any_dt(s: str) -> dt.datetime:
    ...
    s = (s or "").strip()
    # תבנית פשוטה YYYY-MM-DD
    if re.fullmatch(r"\d{4}-\d{2}-\d{2}", s):
        d = dt.datetime.strptime(s, "%Y-%m-%d")  # <-- תוקן כאן
...
def is_trusted_domain(dom: str, trusted_list: List[str]) -> bool:
    """אמון גם בתת-דומיינים (למשל *.bezeq.co.il)."""
    dom = (dom or "").lower()
...
def is_http_allowed(dom: str, allowed_http_list: List[str]) -> bool:
    """היתר לעבוד עם http (לא https) עבור דומיינים מוגדרים."""
    dom = (dom or "").lower()
    ...
    for u in urls:
        ...
        if not is_https:
            if not (
                ...
                # דוחים קישור לא https שאינו מותר
                continue
...
def municipal_attachment_bonus(
    ...
    if muni_hits > 0 and attachments:
        ...
        if matched_files:
            bonus = 0.18  # בין 0.15–0.2; בחרנו 0.18
...
def fetch_attachments(
    ...
    """משיכת צרופות; אופציונלי contentBytes עבור fileAttachment."""
...
def apply_sender_boost(base_conf: float, frm: Dict, rules: List[str]) -> float:
    """כללים כמו: ['+@good.com','-@bad.com','+exact@addr.com']"""
    if not rules:
        return base_conf
...
def main():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="UTC אם אין טיימזון. דוגמה: 2025-07-01 או 2025-07-01T00:00:00+03:00",
    )
    p.add_argument(
        "--end-date",
        type=str,
        default=None,
        help="סוף טווח (בלעדי). דוגמה: 2025-09-30 או 2025-09-30T00:00:00Z",
    )
    ...
    # כללי שולח: מתחילים מדיפולט, מרחיבים/מחליפים
