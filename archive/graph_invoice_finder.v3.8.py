...
"""
Graph Invoice Finder — v3.8 (Full, with Bezeq Playwright handler)

מה חדש לעומת v3.7.x:
- Handler ייעודי ל- myinvoice.bezeq.co.il באמצעות Playwright:
  * טעינת JS (Flutter), איתור כפתור "להורדה"/Download, לכידת ה-Download event ושמירת PDF.
  * דגלים: --interactive (לפתרון ידני של captcha/כניסה), --no-browser-headless.
- ממשיך לתמוך בנתיב ה"סטטי": חיפוש קישורי PDF ב-HTML + חפירה (onclick/script/iframe/form).
- AQS $search בלבד (ללא $filter) כדי להימנע משגיאות SearchWithFilter.
- הורדת צרופות דרך /$value (ללא @odata.type ב-$select).
- אימות PDF: PyMuPDF (ברירת מחדל) או PyPDF2; --verify none|light|strict + --verify-threshold.
"""
...
# Playwright flags
PLAYWRIGHT_AVAILABLE = None  # נאתחל Lazy
...
def download_bezeq_with_playwright(
    ...
    *,
    ...
    """
    פותח את דף בזק, מאתר כפתור 'להורדה'/Download, לוכד download ושומר PDF.
    דורש playwright + chromium מותקנים.
    """
    from playwright.sync_api import sync_playwright, Page, Download, Response
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_timeout(1500)  # bootstrap

        # אם יש captcha/כניסה והמשתמש ביקש interactive – נצלם מסך ונמתין לפעולה ידנית
        has_captcha = page.locator("iframe[src*='recaptcha']").count() > 0
        logged_gate = any(
            k in page.content() for k in ["g-recaptcha", "captcha", "התחבר", "כניסה"]
        )
        if (has_captcha or logged_gate) and interactive:
            ss_path = out_dir / "bezeq_screen_before_click.png"
            page.screenshot(path=str(ss_path), full_page=True)
            diag["notes"].append(f"screenshot_saved:{ss_path}")
            page.wait_for_timeout(30000)  # עד 30 שניות לפעולה ידנית

        candidates = [
            page.get_by_role("button", name=re.compile("להורדה|הורד|Download", re.I)),
            page.get_by_role("link", name=re.compile("להורדה|הורד|Download", re.I)),
            page.get_by_text(re.compile("להורדה|הורד|Download", re.I)),
        ]
            ...
            for c in candidates:
                try:
                    if c.first.is_visible():
                        ...
                        target = out_dir / sanitize_filename(suggested)
                    ...
                    diag["notes"].append(f"click_try_failed:{type(e).__name__}:{e}")
            page.wait_for_timeout(1000)

        # ניסיון לאתר קישורי PDF ישירים בדף
        links = page.locator("a[href$='.pdf'], a[href*='.pdf?'], a[href*='.PDF']")
        if links.count() > 0:
            try:
                with page.expect_download(timeout=timeout_ms) as dl_info:
                    links.first.click()
                dl = dl_info.value
                target = out_dir / sanitize_filename(
        ...
        ss_path = out_dir / "bezeq_screen_no_pdf.png"
        page.screenshot(path=str(ss_path), full_page=True)
        diag["notes"].append(f"no_pdf_found_post_bootstrap; screenshot:{ss_path}")
        browser.close()
        return None, diag
...
def fetch_invoice_from_link(
    ...
    *,
    ...
    # 1) direct attempt (אם זה .pdf או ct=pdf)
    ...
    # 2) Bezeq special handler (Playwright)
    if (
    ...
    # 3) HTML static path: נוריד HTML ונחפש בו PDF/כפתור/iframe
    try:
