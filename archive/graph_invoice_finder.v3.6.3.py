...
"""
Graph Invoice Finder (v3.6.3)
שינויים עיקריים מ-3.6.2:
- הסרת gov.il מהרשימת המהימנה הדיפולטית; הוספת cardcom.co.il.
- ניקוד לינקים "לפי קיום" במקום צבירה פר-לינק; תקרות חדשות.
- הורדת רעש של דומייני מעקב/שיווק (webversion/web-view/mandrill וכו').
- בונוס חזק לנושא המכיל 'חשבונית'/'קבלה', ובונוס לקבלות תרומה.
- ענישת שמות קבצים מהטיפוס 'הודעה/מידע/הסבר ... חשבונית'.
- שאר השיפורים נשמרו (ארנונה, allowed-http, הסברים, בדיקות קלט וכו').
"""
...
# שמות קבצים "שיווקיים" שמורידים ניקוד אם מופיעים סביב חשבונית/קבלה
...
LINK_RULES = {
    ...
    "trusted_providers": [
        ...
        "wix.com",
        "shopify.com",
        "tax.gov.il",  # ספציפי; gov.il כללי הוסר
        "freshbooks.com",
        "waveapps.com",
        "bezeq.co.il",
        "cardcom.co.il",  # חדש: עבור קבלות/תרומות
    ],
    "heuristics": {
        "path_patterns": [
            "invoice",
        ...
        # צמצום: רק PDF תורם ניקוד "סיומת קובץ"
        "file_like_suffixes": [".pdf"],
    },
    # דומיינים עם HTTP מותר (ללא https)
    "allowed_http_providers": ["citypay.co.il"],
    # דומייני מעקב/שיווק — לא מהימנים, וללא מילות־מפתח/נתיב מתאים לא יתרמו ניקוד
    "tracking_domains": [
        "webversion.net",
        "web-view.net",
        "mandrillapp.com",
...
# ===== AQS strategies (ללא $filter/$orderby/$count) =====
...
STRATEGIES = [
    ...
    # יעדכן דינמית מדומיינים מהימנים
    {
...
def is_tracking_domain(dom: str) -> bool:
    dl = LINK_RULES["tracking_domains"]
    if dom in dl:
        return True
...
# ---- ניקוד לינקים (מחושב "לפי קיום", לא לפי כמות) ----
def score_links(
    ...
    for u in urls:
        u_low = u.lower()
        # https חובה, אלא אם הדומיין ברשימת HTTP מותר
...
# בונוס חזק לנושא עם טריגרים ישירים
...
# ענישה לשמות קבצים מהטיפוס "הודעה/מידע/הסבר ... חשבונית"
...
def main():
    ...
    p.add_argument("--download-attachments", default=None)
    ...
    for mid, msg in seen.items():
        ...
        # ענישה לשמות צרופות "הודעה/מידע ... חשבונית"
        penalty = attachment_notice_penalty(attach_hits)
        conf = min(1.0, max(0.0, conf + penalty))
        if args.explain and penalty:
            row.setdefault("explanations", {})["attachment_notice_penalty"] = penalty

        # Sender boost
        conf = apply_sender_boost(conf, msg.get("from") or {}, sender_rules)

        # Negative keywords (מה-CLI אם סופקו)
