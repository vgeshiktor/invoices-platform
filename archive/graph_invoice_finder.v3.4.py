...
"""
Graph Invoice Finder (v3.4)
- $search without $filter/$orderby/$count (avoid SearchWithFilter & InefficientFilter)
- Date filtering is client-side
- Adds --start-date / --end-date (override lookback)
- Strategy descriptions + per-strategy summary table (candidates & final matches)
- Exports results (JSON/CSV) and summary (CSV)
"""
...
def parse_any_dt(s: str) -> dt.datetime:
    """Parse YYYY-MM-DD or full ISO8601; naive → UTC."""
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"Invalid date '{s}'. Use YYYY-MM-DD or ISO8601. ({e})"
        )
...
def main():
    ...
    # Describe the effective date window for clarity
    win_str = f"{start_dt.isoformat()} → {end_dt.isoformat()} (UTC, end exclusive)"
    print(
        f"\nStrategy summary (window={win_str}, min_conf={DEFAULTS['min_confidence']}):"
    )
    print("-" * 100)
