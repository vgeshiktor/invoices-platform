...
# -*- coding: utf-8 -*-
"""
graph_invoice_finder.v3.9.1.py
==============================

תיקוני v3.9.1:
- אין דריסה של קבצים: שם ייחודי ע"י תג msgId קצר ומונה; לבזק – גם InvoiceId.
- מנגנון dedup ע"פ SHA-256 (בזמן ריצה) – אם זהה בתוכן לקובץ שכבר נשמר, מדלגים.
- סינון הדוק יותר לאי-רלוונטי: הוספת Negative Keywords (למשל תלוש שכר),
  והסרת "statement" מן חיוביות כדי לצמצם false positives.
- אימות PDF עם PyMuPDF: חייב להיות לפחות hit חיובי אחד וללא שלילות.
- הטיית ארנונה/עיריות: מונחי זיהוי ספציפיים כדי לשמר חשבוניות רשות.
...
דוגמאות:
    python graph_invoice_finder.v3.9.1.py \
...
class GraphClient:
    ...
    def iter_messages(
        self, start_iso: str, end_iso: str, page_size: int = 50, max_pages: int = 50
    ):
...
def decide_pdf_relevance(path: str, trusted_hint: bool = False) -> Tuple[bool, Dict]:
    ...
    # אם "חזק" (trusted_hint+ארנונה במרקם הטקסט) אפשר להקל – אבל עדיין אין שלילות:
    if trusted_hint and stats["neg_hits"] == 0 and stats["pos_hits"] == 0:
        # נאפשר שמירה אם אין שלילה ובא מקומם אמין (למשל בזק) – יכוסה ע"י שם ספק
        ok = True
    return ok, stats
...
def main():
    ap = argparse.ArgumentParser(
        description="Graph Invoice Finder v3.9.1 (no overwrite + strict relevance)"
    )
    ...
    saved_rows, rejected_rows, download_report = [], [], []
    seen_hashes: set = set()

    msg_idx = 0
    for msg in gc.iter_messages(start_iso, end_iso, page_size=50, max_pages=50):
        msg_idx += 1
        msg_id = msg.get("id")
        ...
        for a in atts:
            ...
            try:
                ...
                trusted_hint = False  # ל-attch אין דומיין; נסמוך על תוכן+נושא
                ok = True
                stats = {"pos_hits": 1, "neg_hits": 0}
                if args.verify:
            ...
            for u in links:
                ...
                if r:
                    ...
                    trusted_hint = within_domain(
                        u, TRUSTED_PROVIDERS
                    ) or is_municipal_text(subject + " " + preview)
