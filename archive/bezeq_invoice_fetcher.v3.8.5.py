...
# -*- coding: utf-8 -*-
"""
bezeq_invoice_fetcher_v3_8_5.py

מטרה
------
הורדת חשבונית PDF מקישור מייל של בזק (myinvoice.bezeq.co.il), שהוא SPA של Flutter Web.
הסקריפט מתמודד עם:
  • UI שמצויר ב-Canvas/Flutter (כפתור "להורדה" לא תמיד DOM רגיל)
  • הורדות blob / חלון קופץ
  • PDF.js viewer (לחיצה על "לצפיה" ואז כפתור הורדה פנימי)
  • תגובות PDF גם אם content-type הוא application/octet-stream + Content-Disposition
...
דוגמאות הרצה
-------------
# מצב דיבוג: חלון גלוי + trace + צילומי מסך + לוג קצר במסוף
python bezeq_invoice_fetcher_v3_8_5.py \
  --url "https://myinvoice.bezeq.co.il/?MailID=..." \
  --out-dir "./invoices_out_bezeq" \
  --headful --trace --screenshots --verbose

# מצב שקט (ברירת מחדל): headless + trace + screenshots
python bezeq_invoice_fetcher_v3_8_5.py \
  --url "https://myinvoice.bezeq.co.il/?MailID=..." \
  --out-dir "./invoices_out_bezeq"

מה חדש בגרסה זו (v3.8.5)
-------------------------
1) תיקון TypeError במאזין console: שימוש במאפיינים (msg.type, msg.text) עם fallback אם זו מתודה.
2) זמן מודע-UTC במקום datetime.utcnow() (מטפל באזהרת Deprecation).
3) קליק לפי semantics של Flutter (flt-semantics) וביצוע mouse.click על קואורדינטות.
4) לכידת PDF: גם application/pdf וגם application/octet-stream + filename.pdf ב-Content-Disposition.
5) מאזיני הורדה + קונסול + pageerror מחוברים לכל דף (כולל popups) אוטומטית.
6) דגלים סימטריים: --trace/--no-trace, --screenshots/--no-screenshots, --headful/--headless, ועוד --verbose.

פלט
----
הסקריפט מדפיס JSON בסיום עם שדות:
  { "ok": bool, "path": str|None, "url": str, "out_dir": str, "notes": [ ... ] }
קוד יציאה: 0 בהצלחה, 2 אם לא נשמר PDF (נוח לשילוב בפייפליין).

טיפים
-----
• אין צורך לברוח ? ו- & אם ה-URL במרכאות.
• אם עדיין לא יורד, הרץ עם --headful --trace --screenshots --verbose ושלח את:
  - out/screens/*
  - out/bezeq_trace_*.zip
  - ה-JSON שמודפס למסוף
"""
...
# תוויות בעברית ליוניקוד (למקרה שמנוע הסלקטורים צריך)
...
def now_stamp() -> str:
    # זמן מודע-UTC (מחליף utcnow() המיושן)
...
def safe_screenshot(
    ...
    """מצלם מסך עם Fallbacks בלי להפיל את הריצה."""
...
def scan_dom_for_hebrew_and_click(page, words_regex: str) -> bool:
    """חיפוש טקסטים בדום ולחיצה JS (למקרה שאין סלקטור ARIA תקין)."""
    try:
        found = page.evaluate(
            """(pattern) => {
...
def click_by_semantics_label(
    ...
    """
    Flutter Web (CanvasKit/HTML-renderer) משתמש ב-flt-semantics.
    כאן מאתרים אלמנטים עם aria-label תואם ומבצעים mouse.click על מרכז ה-BBox.
    """
    try:
        data = page.evaluate(
            """(labels) => {
...
def is_pdf_like_response(resp) -> bool:
    """בודק PDF לפי content-type, סיומת URL, או octet-stream+Content-Disposition."""
    try:
...
def attach_page_listeners(page, *, downloads_bucket: list, note):
    """מאזינים לדף יחיד: download/console/pageerror."""
...
def fetch_bezeq_pdf(
    ...
    with sync_playwright() as pw:
        ...
        # מאזיני תגובה (לכידת PDF) ברמת קונטקסט – תופס גם חלונות קופצים
        pdf_responses: List = []
        ...
        # נחבר גם מאזיני הורדה/קונסול לכל דף חדש
        downloads: List = []
        ...
        # --- ניסיונות לחיצה על "להורדה" / "לצפיה" ---
        clicked = (
        ...
        # השהייה קצרה כדי לאפשר לפלואו ההורדה/פתיחת הצופה להתבצע
        time.sleep(1.0)

        # --- הורדה מאירוע download ---
        ...
        # --- אם מדובר ב-PDF.js, נסה את כפתור ההורדה של הצופה ---
        if not run["ok"]:
...
def main():
    ap = argparse.ArgumentParser(
        description="Bezeq (myinvoice) PDF fetcher – Flutter/Blob/PDF.js aware (v3.8.5)"
    )
    ap.add_argument(
        "--url",
        required=True,
    ...
    # headful/headless (בררת מחדל: headless)
    ...
    # screenshots on/off (בררת מחדל: ON)
    ...
    ap.add_argument("--verbose", action="store_true", help="לוג קצר במסוף בזמן ריצה")
