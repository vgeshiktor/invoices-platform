...
# -*- coding: utf-8 -*-
"""
graph_invoice_finder.v3.9.1a.py
===============================

תיקוני v3.9.1a:
- תיקון NameError: הוספת Tuple (וגם Set) ל-imports של typing.
- יתר השינויים מ-3.9.1: מניעת דריסה, dedup לפי SHA-256, סינון תלושי שכר, אימות PDF ע"י PyMuPDF,
  זיהוי ארנונה/עירייה, תמיכה בהורדת בזק דרך API שנחטף מהקונסול/רשת.
...
דוגמאות:
    python graph_invoice_finder.v3.9.1a.py \
...
def decide_pdf_relevance(path: str, trusted_hint: bool = False) -> Tuple[bool, Dict]:
    ...
    # אם "חזק" (trusted_hint+אין שלילה) אפשר להקל (בזק וכד’) – אבל רק אם אין שלילה
...
def main():
    ap = argparse.ArgumentParser(
        description="Graph Invoice Finder v3.9.1a (no overwrite + strict relevance)"
    )
