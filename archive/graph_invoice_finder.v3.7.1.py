...
"""
Graph Invoice Finder (v3.7.1)

שדרוגים לעומת v3.7:
- תמיכה ב-PyMuPDF (fitz) כמנוע חילוץ טקסט מ-PDF, עם פרמטר --pdf-engine (auto/pymupdf/pypdf).
- נפילה אחורה אוטומטית למנוע חלופי אם המנוע המבוקש לא מותקן.
- שאר יכולות v3.7 נשמרו: הורדת צרופות, זיהוי/הורדה מלינקים, אימות שהתוכן הוא חשבונית/קבלה,
  סינון דומיינים שיווקיים, רשימת ספקים מהימנים, דו"ח הורדות, ועוד.
"""
...
# --- HTML parsing (אופציונלי לשיפור חיפוש קישורי PDF בדפי HTML) ---
try:
...
DEFAULTS = {
    ...
    "timeout_sec": 30,
    "http_timeout": 25,
    "min_pdf_size_bytes": 6_000,  # סינון קבצים קטנים מדי
}
...
LINK_RULES = {
    ...
    "trusted_providers": [
        ...
        "wix.com",
        "shopify.com",
        "tax.gov.il",  # השארנו ספציפי; gov.il כללי הוסר
        "freshbooks.com",
        "waveapps.com",
        "bezeq.co.il",
        "cardcom.co.il",  # תרומות/קבלות
    ],
    "heuristics": {
        "path_patterns": [
            "invoice",
        ...
        "file_like_suffixes": [".pdf"],  # נוריד רק PDF מלינקים
    },
...
# ===== אסטרטגיות (AQS בלבד; ללא $filter/$orderby/$count) =====
STRATEGIES = [
    {
...
# ---- ניקוד לינקים (לפי קיום, לא לפי כמות) ----
def score_links(
    ...
    for u in urls:
        ...
        if dom_trust or has_kw or has_pat or has_suf:
            kept.append(u)
            any_trust = any_trust or dom_trust
            any_kw_or_pat = any_kw_or_pat or has_kw or has_pat
            any_suf = any_suf or has_suf
...
# =========================
# אימות PDF כחשבונית — עם PyMuPDF כעדיפות
# =========================
...
def extract_pdf_text_first_pages(path: str, max_pages: int = 2) -> str:
    """
    חילוץ טקסט מהעמודים הראשונים של PDF.
    עדיפות ל-PyMuPDF (fitz) — מהיר ועם תמיכה טובה בעברית.
    נפילה אחורה ל-PyPDF2 אם PyMuPDF לא זמין/נכשל (או אם נבחר מנוע אחר).
    """
    # נסיון עם PyMuPDF
...
def verify_pdf_is_invoice(
    path: str, *, min_size: int = DEFAULTS["min_pdf_size_bytes"]
    ...
    ok = score >= 0.45
    return ok, {"size": sz, "text_score": round(score, 3)}
...
def download_pdf_from_url(
    sess: requests.Session, url: str, out_path: str
...
def fetch_invoice_from_link(
    ...
    """
    מחזיר path אם נשמר PDF מאומת כחשבונית, אחרת None.
    """
    u = url.strip()
    dom = domain_of(u)
    ...
    ok, why, meta = download_pdf_from_url(sess, u, pdf_out)
    if ok:
        ok2, meta2 = verify_pdf_is_invoice(pdf_out)
        if ok2:
    ...
    if why in ("not_pdf", "ok"):
        try:
            ...
            for j, p in enumerate(pdf_links, start=1):
                ...
                ok3, why3, meta3 = download_pdf_from_url(sess, p, out2)
                if ok3:
                    ok4, meta4 = verify_pdf_is_invoice(out2)
                    if ok4:
                        return out2, {
                            "download": meta3,
                            "verify": meta4,
...
def main():
    ...
    # בריאות מנועי PDF: אם ביקשו מנוע שאינו מותקן — נפילה אחורה והודעה
    if PDF_ENGINE == "pymupdf" and not HAS_PYMUPDF:
        ...
        if HAS_PYPDF:
            ...
            PDF_ENGINE = "auto"  # לא זמין — ננסה auto (לא ימצא מנוע)
    ...
    # עדכון דינמי של אסטרטגיית ספקים
    for s in STRATEGIES:
    ...
    for mid, msg in seen.items():
        ...
        # משיכת צרופות (ללא תוכן בשלב הניקוד)
        attach_hits: List[Dict] = []
        ...
        muni_bonus, muni_info = municipal_attachment_bonus(subj, attach_hits)
        ...
        conf, subj_hits = apply_subject_regex_boost(conf, subj, subject_regexes)
                ...
                for a in full_atts:
                    ...
                    if ct.lower() == "application/pdf" or name.lower().endswith(".pdf"):
                        ...
                        with open(tmp_path, "wb") as f:
                            f.write(raw)
                        ok, meta = verify_pdf_is_invoice(tmp_path)
                        if ok:
