...
"""
Graph Invoice Finder (full version, InefficientFilter-safe)
- Auth: MSAL Device Code (PublicClientApplication)
- Targets: /v1.0/me/messages
- Uses $search for Heb/Eng keywords; keeps $filter light (date only) to avoid InefficientFilter
- Omits $orderby and $count whenever $search is present; sorts client-side
- Paginates via @odata.nextLink
- Fetches attachments for candidates; extracts & scores links
- Outputs JSON + CSV

Example:
  python graph_invoice_finder.py --client-id "<APP_ID>" --out-json invoices.json --out-csv invoices.csv
"""
...
DEFAULTS = {
    ...
    "page_size_search": 50,  # smaller for $search
    "page_size_plain": 100,  # used if we ever call without $search
    "max_pages": 20,
    "lookback_days": 120,
    "min_confidence": 0.55,
    "timeout_sec": 30,
...
STRATEGIES = [
    # Keep server filter LIGHT (date only); hasAttachments handled client-side to avoid InefficientFilter
    {
...
def run_strategy(
    ...
    params = {
        ...
        "$top": str(page_size),
        "$filter": strategy["filter_tpl"].format(since=since_iso),
        "$search": strategy["search"],
        # $orderby and $count intentionally omitted to avoid InefficientFilter
    }
    out, pages = [], 0
    ...
    while next_url and pages < max_pages:
        ...
        next_params = {}  # nextLink contains everything
